function [X, gamma_ind, gamma_est, count] = tMFOCUSS(Phi, Y, lambda, varargin)
% tMFOCUSS algorithm for the MMV model with temporally correlated source vectors
%    
% ============================== INPUTS ============================== 
...
%       = tMFOCUSS(Phi,Y,lambda,'p',0.8,'prune_gamma',1e-4,'max_iters',500,'epsilon',1e-8,'print',0);
%
% [Example 2]
%      lambda = 1e-5;  
%      [X,gamma_ind,gamma_est,count] = tMFOCUSS(Phi,Y, lambda);
%
% ==============================  Reference =============================
%   [1] Zhilin Zhang, Bhaskar D. Rao, Iterative Reweighted Algorithms for
%       Sparse Signal Recovery with Temporally Correlated Source Vectors,
%       ICASSP 2011
%
%   [2] Zhilin Zhang, Bhaskar D. Rao, Sparse Signal Recovery with Temporally 
%       Correlated Source Vectors Using Sparse Bayesian Learning,IEEE
%       Journal of Selected Topics in Signal Processing (Special Issue on 
%       Adaptive Sparse Representation of Data and Applications in Signal 
...
if (PRINT) fprintf('\nRunning tMFOCUSS for the MMV Problem...\n'); end
...
while (1)
    ...
    % When number of nonzero gamma_i is less than N, no need to learn B.
    % For reasons, see the above Ref.[2] (experiment part).
    if m >= N
        B = zeros(L,L);
        for i =  1 : m
